--[[
VOTING NPC WITH FIXED SIZE HOLOGRAM
...
- Fixed size hologram that doesn't scale with distance
- Only visible when close to the NPC
...
local DISPLAY_VISIBILITY_DISTANCE = 15 -- Maximum distance to see the display (studs)
local HOLOGRAM_HEIGHT = 3.5 -- Height above NPC
local UPDATE_INTERVAL = 0.5 -- How often to refresh the display

print("=== Fixed Size Hologram Voting NPC Starting ===")
...
local function createVoteDisplay()
    ...
    billboardGui.SizeOffset = Vector2.new(0, 0) -- No size scaling with distance
    billboardGui.ExtentsOffset = Vector3.new(0, HOLOGRAM_HEIGHT, 0) -- Position above NPC
    billboardGui.ExtentsOffsetWorldSpace = Vector3.new(0, 0, 0) -- No world space offset
    billboardGui.AlwaysOnTop = false -- Don't always show on top
    
    -- Attach to NPC's PrimaryPart
    ...
    print("Fixed size vote display created")
    return billboardGui
end
...
local function initializeVotingSystem()
    ...
    print("=== Fixed Size Hologram Voting NPC Initialized ===")
end
