--[[
ENHANCED VOTING NPC WITH IMPROVED VISIBLE DISPLAY
...
This script fixes:
- Enhanced visibility of the hologram
- Always visible vote count with emojis
...
local DISPLAY_RADIUS = 50 -- Increased visibility radius (was 7)
local HOLOGRAM_HEIGHT = 3.5 -- Lowered height to be more visible
local UPDATE_INTERVAL = 0.5 -- How often to refresh the display

print("=== Enhanced Voting NPC System Starting ===")
...
local function createVoteDisplay()
    ...
    billboardGui.Size = UDim2.new(0, 300, 0, 60) -- Larger size for better visibility
    ...
    bgFrame.BackgroundTransparency = 0.6 -- Semi-transparent background
    ...
    print("Enhanced vote display created with improved visibility")
    return billboardGui
end
...
function updateVoteText(textLabel)
    ...
    textLabel.Text = likesEmoji .. " " .. voteData.likes .. " | " .. 
                      dislikesEmoji .. " " .. voteData.dislikes
    ...
    -- Print to verify display is updating
    print("Updated vote display: 👍 " .. voteData.likes .. " | 👎 " .. voteData.dislikes)
end
...
local function updateVoteDisplay()
    ...
    local textLabel = voteData.voteDisplay:FindFirstChild("Background"):FindFirstChild("VoteCountLabel")
    if textLabel then
        updateVoteText(textLabel)
    else
...
local function initializeVotingSystem()
    ...
    print("=== Enhanced Voting NPC System Initialized ===")
end
